<?php
  ...
  * */
	...
	abstract class User {
		...
		private static $_db;	/** @var bool $_init	Initialization flag */
		private static $_init;
		
		/** @var array $_definition	Definition of DB users table */
		private static $_definition = array(
			'table' => 'administradores',/* users table name */
			'id' => 'id', /* user identifier field name */
			'login' => 'usuario', /* user login field name*/
			'pass' => 'passwd',	/* user password field name */
			'key' => 'session_key',	/* user session key field name */
			'fields' => array('usuario', 'id')		/* add your fields here */
		);
		...
		static function getByID($userID) {
			...
			if(self::$_init) {
				...
				if($lot = $res->fetch()) {
					...
					/* other fields */
					syslog(LOG_INFO, __FILE__ . " - definition: ".serialize($_definition) ) ;
		...
		*/		
		static function passwordCheck($pass, $hash) {
			...
			if($hash == $salt . md5($pass . $salt)) {$result = true;}*/
			return $result;
		}
		...
		*/
		static function login($login, $pass) {
			...
			$sql = 'SELECT `' . self::$_definition['id'] . '`, `' . self::$_definition['pass'] . '` FROM `' . 
				self::$_definition['table'] . '` WHERE `' . self::$_definition['login'] . '` = :login;';
